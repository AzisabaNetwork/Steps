package net.azisaba.steps.light;
 ...
 *
 * https://github.com/UnrealValentin/MinestomBasicLight
 */
    ...
    chunks.forEach((chunk -> {
      ...
      for (Section section : sections) {
        ...
        for (int x = 0; x < 16; x++) {
          for (int z = 0; z < 16; z++) {
            for (int y = 15; y > -1; y--) {
              if (!utils.lightCanPassThrough(
                  Block.fromStateId((short) section.blockPalette().get(x, y, z)))) {
                exposed[x][z] = false;
              }
              if (exposed[x][z]) {
